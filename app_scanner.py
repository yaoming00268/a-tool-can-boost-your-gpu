...
class AppScanner:
    ...
    def _resolve_executable(cls, display_name, display_icon, install_dir, uninstall_str):
        ...
        if not exe_path and uninstall_str:
            clean_unins = os.path.expandvars(uninstall_str.strip('"').strip())
            match = re.search(r'([a-zA-Z]:\\[^",\t\r\n]+\.exe)', clean_unins, re.IGNORECASE)
            if match:
                unins_exe = match.group(1)
                unins_dir = os.path.dirname(unins_exe)
                if os.path.exists(unins_dir):
                    try:
                        for f in os.listdir(unins_dir):
                            if f.lower().endswith(".exe") and "unins" not in f.lower() and "setup" not in f.lower():
                                exe_path = os.path.join(unins_dir, f)
                                break
                    except Exception:
                        pass
    ...
    def _scan_powershell(cls):
        ...
        try:
            ...
            res = subprocess.run(["powershell", "-Command", cmd], capture_output=True, text=True, timeout=8)
